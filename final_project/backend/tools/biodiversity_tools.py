...
async def analyze_deforestation(
    ...
    Args:
        location: Nombre de la ubicación (ej: "Amazonas", "Borneo", "Parque Nacional Yasuní")
    ...
    try:
        ...
        return {
            "success": False,
            "error": str(e),
            "message": "Error analizando deforestación. Verifica los parámetros."
        }
...
async def detect_illegal_fishing(
    ...
    Args:
        marine_region: Nombre de la región marina (ej: "Galápagos", "Great Barrier Reef")
    ...
    """
    try:
        result = await ocean_service.detect_illegal_trawling(marine_region, radius_km)
        ...
        if result.get("assessment", {}).get("risk_level") == "HIGH":
            alert_obj = alert_service.create_alert(
                ...
                description="Actividad de pesca ilegal detectada en área protegida",
        ...
        return {
            "success": False,
            "error": str(e),
            "message": "Error detectando actividad de pesca. Verifica la región."
        }
...
async def monitor_wildlife(
    ...
    Args:
        park_name: Nombre del parque nacional (ej: "Serengeti", "Kruger", "Yasuní")
    ...
    try:
        if species:
            ...
            result = await wildlife_service.get_species_activity(park_name, species)
            return {
                "success": True,
                "data": result,
                ...
                for alert in result["assessment"]["alerts"]:
                    if alert.get("type") == "POTENTIAL_POACHING":
                        alert_obj = alert_service.create_alert(
                            ...
                            description=alert.get("description", "Actividad sospechosa detectada"),
