...
class SatelliteService:
    ...
    async def get_deforestation_alert(
        ...
        try:
            ...
            if self.api_key:
                # Ejemplo de llamada real a NASA GIBS (implementación completa 
                # requeriría autenticación OAuth)
                # url = f"{self.NASA_GIBBS_ENDPOINT}/
            ...
            if response_data["forest_cover_change"]["change_percent"] < -3.0:
                response_data["alerts"].append({
                    ...
                    "severity": "HIGH" if response_data["forest_cover_change"]
                    ["change_percent"] < -5.0 else "MEDIUM",
                    "description": f"Se detectó pérdida de cobertura forestal 
                    " del {abs(response_data['forest_cover_change']['change_percent']):.1f}%"
                })
