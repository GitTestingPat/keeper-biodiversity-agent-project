...
class AlertService:
    ...
    def __init__(self):
        self.api_key = settings.OPENAI_API_KEY  # Para generación de resúmenes con AI
    ...
    def create_alert(
        ...
        """
        return {
            "id": f"ALT-{datetime.now().strftime('%Y%m%d%H%M%S')}-{hash(description) % 10000:04d}",
    ...
    async def _send_email(self, alert: dict) -> dict:
        ...
        print(f"   📧 Email simulado enviado a: {', '.join(self.email_recipients)}")
        print(f"      Asunto: [{alert['severity'].upper()}] {alert['type']} - {alert['location'].get('name', '')}")
