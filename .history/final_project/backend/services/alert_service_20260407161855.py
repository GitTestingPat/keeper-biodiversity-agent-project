...
class AlertService:
    ...
    def create_alert(
        ...
        return {
            "id": (
                f"ALT-{datetime.now().strftime('%Y%m%d%H%M%S')}-"
                f"{hash(description) % 10000:04d}"
            ),
    ...
    async def send_alert(
        self, alert: dict, channels: Optional[List[str]] = None
    ) -> dict:
        """Envía una alerta a los canales configurados.

        Args:
            alert: Dict de alerta creado con create_alert()
            channels: Lista de canales ['webhook', 'email', 'log'].
                             Default: todos configurados

        Returns:
            Dict con resultado del envío
        """
        # Por defecto, solo log en modo demostración
    ...
    async def _log_alert(self, alert: dict) -> dict:
        ...
        location_name = alert["location"].get("name", "Desconocida")

        print(f"\n{emoji} ALERTA [{severity_upper}]")
        print(f"   ID: {alert['id']}")
    ...
    async def _send_email(self, alert: dict) -> dict:
        ...
        recipients_str = ", ".join(self.email_recipients)
        severity_upper = alert["severity"].upper()
        alert_type = alert["type"]
        location_name = alert["location"].get("name", "")

        print(f"   📧 Email simulado enviado a: {recipients_str}")
        print(
            f"      Asunto: [{severity_upper}] "
            f"{alert_type} - {location_name}"
        )
    ...
    async def generate_ai_summary(self, alert: dict) -> str:
        ...
        if not self.api_key:
            return (
                f"Resumen no disponible (API Key no configurada). "
                f"Alerta: {alert['description']}"
            )
        ...
        try:
            ...
            prompt = (
                "Eres un asistente especializado en conservación de "
                "biodiversidad.\n"
                "Genera un resumen ejecutivo CONCISO y PROFESIONAL para "
                "autoridades ambientales basado en esta alerta:\n\n"
                f"Tipo: {alert['type']}\n"
                f"Severidad: {alert['severity']}\n"
                f"Ubicación: {alert['location']}\n"
                f"Descripción: {alert['description']}\n"
                f"Metadata: {alert.get('metadata', {})}\n\n"
                "El resumen debe:\n"
