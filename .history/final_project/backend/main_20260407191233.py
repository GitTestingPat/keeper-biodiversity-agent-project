...
async def chat(req: ChatRequest):
    """Endpoint de chat que ejecuta el agente con herramientas (Bucle Manual)"""
    try:
        # 1. Importar lo necesario aquí para evitar errores globales
        from langchain_core.messages import HumanMessage, ToolMessage

        # Obtener las herramientas reales del registry
        tools_list = TOOLS_REGISTRY.get("tools", []) if isinstance(TOOLS_REGISTRY, dict) else TOOLS_REGISTRY
        ...
        for _ in range(3):
            ...
            # B. Si NO hay llamadas a herramientas, hemos terminado (respuesta final)
            ...
            for tool_call in response.tool_calls:
                ...
                print(f"🛠️ Ejecutando herramienta: {tool_name} con args: {tool_args}")

                if tool_name in tool_map:
                    # Ejecutar la herramienta (asegúrate que sean async o usa asyncio.run)
                    ...
                    try:
                        ...
                        if hasattr(tool_func, 'coroutine'):
                            ...
                            # Si es una función normal decorada con @tool, a veces necesita invoke directo
                            # Ojo: LangChain tools suelen ser runables.
                            # Si falla, prueba: result = await tool_func.arun(**tool_args)
                            result = await tool_func.ainvoke(tool_args)
